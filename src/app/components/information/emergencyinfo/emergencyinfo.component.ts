...
import EmergencyInfoModel from '../../../models/emergencyinfo.model';
...
@Component({
  selector: 'app-root',
  templateUrl: './emergencyinfo.component.html',
  styleUrls: ['./emergencyinfo.component.scss']
})
      ...
      this.hubService.getContent(Global.idEvent, this.language,Views.nextEventsView)
        .subscribe(data => {
          ...
          if (data.status === Global.responseOk) {
            console.log(this.emergencyInfo)
            this.emergencyInfo = data.data as NextEventModel;
          }  
        });
    } catch (e) {
